...
class FCViewController: UIViewController, UINavigationControllerDelegate, FUIAuthDelegate {
    ...
    func configureDatabase() {
        ...
        _refHandle = ref.child("messages").observe(.childAdded) { (snapshot: DataSnapshot) in
            ...
            print(IndexPath(row: self.messages.count-1, section: 0))
    ...
    func fetchConfig() {
        ...
        remoteConfig.fetch(withExpirationDuration: expirationDuration) { (status, error) in
            if status == .success {
                ...
                // replaces currently cached remote config with the one that was just fetched
                self.remoteConfig.activate()
                let friendlyMsgLength = self.remoteConfig["friendly_msg_length"]
                // we ensure that friendly msg length came from remote configuration and not from the default variable in the app
...
extension FCViewController: UIImagePickerControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        print("imagePickerController triggered")
        // constant to hold the information about the photo
        if let photo = info[UIImagePickerController.InfoKey(rawValue: UIImagePickerController.InfoKey.originalImage.rawValue)] as? UIImage, let photoData = photo.jpegData(compressionQuality: 0.8) {
